...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6, constant=12.7, weight=2):
    assert mode in ['iou', 'iof', 'giou', 'normalized_giou', 'ciou', 'diou', 'nwd',
                    'dotd', 'kl', 'wd'], f'Unsupported mode {mode}'
    ...
    # 'kl' and 'wd' as defined in RFLA (github.com/Chasel-Tsui/mmdet-rfla)
    if mode in ['kl', 'wd']:
        center1 = (bboxes1[..., :, None, :2] + bboxes1[..., :, None, 2:]) / 2
        center2 = (bboxes2[..., None, :, :2] + bboxes2[..., None, :, 2:]) / 2
        whs = center1[..., :2] - center2[..., :2]
        ...
        if mode == 'kl':
            kl = (w2 ** 2 / w1 ** 2 + h2 ** 2 / h1 ** 2 + 4 * whs[..., 0] ** 2 / w1 ** 2 +
                  4 * whs[..., 1] ** 2 / h1 ** 2 + torch.log(w1 ** 2 / w2 ** 2) +
                  torch.log(h1 ** 2 / h2 ** 2) - 2) / 2
            return 1 / (1 + kl)
