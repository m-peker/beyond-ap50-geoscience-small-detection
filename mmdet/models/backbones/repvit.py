...
default_cfgs = generate_default_cfgs(
    {
        ...
        'repvit_ldy.dist_450e_in1k': _cfg(
            hf_hub_id='timm/',
        ),
        'repvit_ldy1.dist_450e_in1k': _cfg(
            hf_hub_id='timm/',
        ),
        'repvit_ldy2.dist_450e_in1k': _cfg(
            hf_hub_id='timm/',
        ),
    }
)
...
def repvit_ldy(pretrained=False, **kwargs):
    """
    Constructs a RepViT-M2.3 model
    """
    model_args = dict(embed_dim=(56, 112, 224, 448), depth=(4, 4, 2, 1))
    return _create_repvit('repvit_ldy', pretrained=pretrained, **dict(model_args, **kwargs))


@BACKBONES.register_module()
def repvit_ldy1(pretrained=False, **kwargs):
    """
    Constructs a RepViT-M2.3 model
    """
    model_args = dict(embed_dim=(56, 112, 224, 448), depth=(6, 6, 2, 1))
    return _create_repvit('repvit_ldy1', pretrained=pretrained, **dict(model_args, **kwargs))


@BACKBONES.register_module()
def repvit_ldy2(pretrained=False, **kwargs):
    """
    Constructs a RepViT-M2.3 model
    """
    model_args = dict(embed_dim=(64, 128, 256, 512), depth=(4, 4, 2, 1))
    return _create_repvit('repvit_ldy2', pretrained=pretrained, **dict(model_args, **kwargs))



class RepVit1(nn.Module):
    def __init__(
        self,
        in_chans=3,
        img_size=224,
    ...
    def forward(self, x):
        ...
        for i in range(len(self.stages)):
            if i == 0:
                ...
                downsample = self.stages[i].downsample
                x = downsample.pre_block(x)
                feats.append(x)
                x = downsample.spatial_downsample(x)
                x = downsample.channel_downsample(x)
                identity = x
                x = downsample.ffn(x)
                x = x + identity
                feats.append(x)

            for j in range(len(self.stages[i].blocks)):
                x = self.stages[i].blocks[j](x)
                feats.append(x)
...
@BACKBONES.register_module()
def repvit_ldy3(pretrained=None, out_indices = [4, 10, 14, 16], **kwargs):
    """
    Constructs a RepViT-M2.3 model
    """
    model_args = dict(embed_dim=(64, 128, 256, 512), depth=(4, 4, 2, 1))
    return RepVit1(pretrained=pretrained, **dict(model_args, **kwargs))
